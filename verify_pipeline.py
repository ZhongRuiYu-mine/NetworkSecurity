...
用法（工作目录 = 仓库根 D:\\NETWORK）：
    D:\\PycharmProjects\\pythonProject\\.venv\\Scripts\\python.exe verify_pipeline.py
    ... python verify_pipeline.py --data real        # 用真实 CICIDS2017 CSV（较慢）
    ... python verify_pipeline.py --detectors if,kalman,autoencoder,null
...
def check_maddpg(env, episodes: int = 3):
    ...
    # 确定性评估 + 存档往返（写到工作区内的 outputs/，避免沙箱里的临时目录权限问题）
    obs = env.reset()
    det_actions, det_cls = agent.select_actions(obs, deterministic=True)
    assert det_actions.shape == (env.num_agents, env.action_dim)
    import os

    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "outputs", "_smoke")
